...
// XXX 加粗**和Font同时使用会出现渲染bug
function Font({children, color}: {children: any; color?: string}) {
  switch (color) {
    case 'red':
      return (
        <span className="text-red-500 bg-red-100 rounded px-1">{children}</span>
      );

    default:
      return (
        <span className="text-black-500 bg-red-100 rounded px-1">
          {children}
        </span>
      );
  }
